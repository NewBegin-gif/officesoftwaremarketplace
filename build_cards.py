...
.tools-grid,.how-grid,.social-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
.tools-grid,.social-grid{grid-template-columns:minmax(0,1fr)}
@media(max-width:960px){.tools-grid,.how-grid,.social-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
...
CARD = """\
<article class="tool-card ledger fade-in" data-category="{category_attr}"{flag_attr}>
 <div class="lg-wie">
  ...
  <div class="lg-naam"><h3>{name}</h3><span class="tool-badge pick">{badge}</span>{alt_row}</div>
 </div>
 <div class="lg-prijs">{price_row}</div>
 <div class="lg-oordeel"><p class="tool-desc">{desc}</p>{best_row}{catch_row}</div>
 <div class="lg-signalen">{flags_row}{coupon_row}<div class="tool-tags">{tags}</div></div>
 <div class="lg-acties tool-cta-row">
...
SNEDE = {
    ...
    "Finance",
    "HR & People",
    "Business Operations",   # 4 sep 2026, Daan: hoort bij kantoor
    "Project Management",
    "Security",
}
...
# Bijna-dubbele chips samentrekken (4 sep 2026, Daan: "Finance, Finance &
# Accounting en Financial Operations zijn hetzelfde onderwerp"). Dit verandert
# alleen het etiket op de kaart en de chip -- de snede hierboven bepaalt nog
# steeds welke tool op deze zuster hoort, en het dossier blijft ongemoeid.
SAMENVOEGEN = {
    'Finance': 'Financial Operations',
    'Finance & Accounting': 'Financial Operations',
    'Productivity': 'IT & Productivity',
}


def snede_naam(cat):
    return SAMENVOEGEN.get(cat, cat)
...
def coupon_row(e):
    c = e.get("coupon")
    if not c:
        return '\n <div class="tc-coupon leeg" aria-hidden="true"></div>'
    code = f'<span class="cd">{html.escape(c["code"])}</span>' if c.get("code") else ""
    return (f'\n <div class="tc-coupon" title="Reader code, verified with the vendor">'
            f'<span aria-hidden="true">&#127915;</span>{code}<span class="lb">{html.escape(c.get("label", ""))}</span></div>')


KOP = '<div class="lg-kop" aria-hidden="true"><span>Tool</span><span>Price, dated</span><span>What it does · best for · the catch</span><span>Pricing patterns · reader code</span><span></span></div>'
...
def main():
    ...
    for t in tools:
        t["category"] = snede_naam(t.get("category"))
    ...
    cards = "\n".join(
        CARD.format(
            ...
            coupon_row=coupon_row(extra.get(t["name"], {})),
